...
```bash
# 推荐精确 pin commit(SHA 以 Release receipt 发布的为准);bash 中整体引号包裹(& 为 shell 特殊字符)
...
PowerShell 中同样将整条 spec 用双引号包裹:
`dsh plugin --profile web add "github:akirajoey/insuremo-dsh#<commit-sha>&path:git-dist/icomposer-workbench"`。

该目录是仓库内**跟踪的预构建产物**(仅 lib/assets、cordis.patch.yml、README、无
scripts 的瘦 package.json),`pnpm pack:git-dist` 原子刷新、`pnpm check:git-dist`
强制与源码零漂移;安装为实体拷贝,无 prepare/allowBuilds。旧写法
`github:<org>/insuremo-dsh#path:packages/icomposer-workbench-dist` 指向**开发源码
目录**(未构建、含 scripts/devDeps),安装即失败,请勿使用。
...
- id: icomposer-workbench
  ...
  inject: [subprocess, storageDomain, workspaceRegistry, skills, webServer, tools, jobs, directoryPicker]
```
