...
| 业务解释 | `ici_explain` 只准备 schema-3 调用链/源码范围并创建 awaiting-input Job；单 API 使用 `query`，group/多 API 必须一次使用 `queries`（2–10，自动去重）创建一个 batch card，一次确认 workspace-relative reference、provider/model 与 not-before；Host 按全局最大并发（默认 4，可在卡片内调整为 1–32，全部 ICI Explain 任务共享、保存后立即生效并重启记忆）调度运行 batch jobs。成功后 confirmation card 位于当前 DSH Web conversation 的这个 `ici_explain` tool call 位置，而不是最终回复末尾；部署新 bundle 后旧标签页需 hard refresh。会话中的 keyed toolview 卡片通过 Host 原生选择器分别选择 workspace-relative supported text 文件或目录（参考资料可选：默认不使用，可选文件/目录或保持 none），不再渲染卡片内树形/modal。资料仅允许 `.md/.txt/.json/.yaml/.yml/.csv/.log`，不支持浏览器上传；空模型 catalog 仍可手输 opaque model ID。到所属 Agent 下一次 idle 后，Host 启动仅有 list/read/submit 三个工具的 fresh Explain Agent；目录目标可浏览后代，文件目标只能读取所选文件；submit 才发布 immutable `finals/<jobId>.json`，随后 `state.json` 最后指向它并点亮第三 Intelligence 图标。 |
...
Host routes：单 Job 的 `GET .../jobs/:jobId/status` / `GET .../jobs/:jobId/folder?path=...`（旧兼容）/ `POST .../jobs/:jobId/native-pick|confirm|cancel|retry`；Batch 增加 `GET .../batches/:batchId/status` 与 `POST .../batches/:batchId/native-pick|confirm|cancel|retry`，一次确认会排入多个 Job，由 Host 全局并发上限（默认 4，卡片内可调 1–32，全部 ICI Explain 批次共享）调度；`GET .../jobs/:jobId/status` 与 `GET .../batches/:batchId/status` 返回 `scheduler: {maxConcurrent, inFlight}`，`POST .../settings` 写入该上限（严格校验 1–32 整数，保存失败不改变旧值）。批量卡片每页显示 5 条任务、页头常驻全批次统计，翻页不改变确认/取消整批/重试失败任务的全批次语义。卡片通过 Host 原生选择器得到 `{path,kind:file|directory}`，原生选择器只在 Host 内短暂处理 absolute path，成功响应和 confirm 仅含 workspace-relative target；取消/不可用/越界/符号链接/不支持类型均返回固定错误且不改变当前选择。旧 Job 的 `folderPath` 目录字段只作读取迁移兼容。
...
8. ICI Explain 在单进程内串行 prepare/final/cancel 并在 submit 前重哈希资料；Explain 任务按当前 Host 实例的全局并发上限（默认 4，1–32）调度，该上限不跨进程——多个 Host 进程各自持有自己的上限；同一 OS 用户的其他进程仍可并发修改 workspace（TOCTOU 不在 MVP 的 `openat` 威胁模型内）；
