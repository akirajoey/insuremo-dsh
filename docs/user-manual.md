...
| 业务解释 | `ici_explain` 只准备 schema-3 调用链/源码范围并创建 awaiting-input Job；单 API 使用 `query`，group/多 API 必须一次使用 `queries`（2–10，自动去重）创建一个 batch card，一次确认 workspace-relative reference、provider/model 与 not-before，闲时逐个运行 batch jobs。成功后 confirmation card 位于当前 DSH Web conversation 的这个 `ici_explain` tool call 位置，而不是最终回复末尾；部署新 bundle 后旧标签页需 hard refresh。会话中的 keyed toolview 卡片通过 Host 原生选择器分别选择 workspace-relative supported text 文件或目录（参考资料可选：默认不使用，可选文件/目录或保持 none），不再渲染卡片内树形/modal。资料仅允许 `.md/.txt/.json/.yaml/.yml/.csv/.log`，不支持浏览器上传；空模型 catalog 仍可手输 opaque model ID。到所属 Agent 下一次 idle 后，Host 启动仅有 list/read/submit 三个工具的 fresh Explain Agent；目录目标可浏览后代，文件目标只能读取所选文件；submit 才发布 immutable `finals/<jobId>.json`，随后 `state.json` 最后指向它并点亮第三 Intelligence 图标。 |
...
Host routes：单 Job 的 `GET .../jobs/:jobId/status` / `GET .../jobs/:jobId/folder?path=...`（旧兼容）/ `POST .../jobs/:jobId/native-pick|confirm|cancel|retry`；Batch 增加 `GET .../batches/:batchId/status` 与 `POST .../batches/:batchId/native-pick|confirm|cancel|retry`，一次确认会排入多个 Job，闲时逐个运行。卡片通过 Host 原生选择器得到 `{path,kind:file|directory}`，原生选择器只在 Host 内短暂处理 absolute path，成功响应和 confirm 仅含 workspace-relative target；取消/不可用/越界/符号链接/不支持类型均返回固定错误且不改变当前选择。旧 Job 的 `folderPath` 目录字段只作读取迁移兼容。
