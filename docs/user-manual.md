...
| 业务解释 | `ici_explain` 只准备 schema-3 调用链/源码范围并创建 awaiting-input Job；单 API 使用 `query`，group/多 API 必须一次使用 `queries`（2–10，自动去重）创建一个 batch card，一次确认 workspace-relative reference、provider/model 与 not-before，闲时逐个运行 batch jobs。成功后 confirmation card 位于当前 DSH Web conversation 的这个 `ici_explain` tool call 位置，而不是最终回复末尾；部署新 bundle 后旧标签页需 hard refresh。会话中的 keyed toolview 卡片通过 Host 原生选择器分别选择 workspace-relative supported text 文件或目录（默认 `ref_doc`），不再渲染卡片内 tree/modal。资料仅允许 `.md/.txt/.json/.yaml/.yml/.csv/.log`，不支持浏览器上传；空模型 catalog 仍可手输 opaque model ID。到所属 Agent 下一次 idle 后，Host 启动仅有 list/read/submit 三个工具的 fresh Explain Agent；目录目标可浏览后代，文件目标只能读取所选文件；submit 才发布 immutable `finals/<jobId>.json`，随后 `state.json` 最后指向它并点亮第三 Intelligence 图标。 |
