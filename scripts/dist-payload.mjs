...
export const distPackageDir = join(repoRoot, "packages", "icomposer-workbench-dist");
...
export async function buildDistLib() {
  ...
  execFileSync("pnpm", ["run", "build"], { cwd: distPackageDir, stdio: "inherit" });
}
...
async function assertNoRetiredText(root, label) {
  async function walkText(dir) {
    for (const entry of await readdir(dir, { withFileTypes: true })) {
      ...
      if (info.size > 20_000_000) throw new Error(`refusing oversized text scan: ${full}`);
...
export function canonicalizeCssModuleClasses(bytes) {
  ...
  const mapPattern = /(?:var|const|let) ([A-Za-z0-9_$]+)_module_css_default = \{\n([\s\S]*?)\n\};/gu;
  const replacements = new Map();
  let map;
  while ((map = mapPattern.exec(text)) !== null) {
    const members = [...map[2].matchAll(/"([^"]+)": "([^"]+)"/gu)];
    const classes = members.map(member => {
      const local = member[1];
      return member[2].endsWith(`_${local}`) ? { old: member[2], local } : null;
    }).filter(value => value !== null);
    if (classes.length === 0 || classes.length !== members.length) continue;
    const signature = `${map[1]}\n${classes.map(value => value.local).sort().join("\n")}`;
    const prefix = `wb${createHash("sha256").update(signature).digest("hex").slice(0, 8)}`;
    for (const value of classes) replacements.set(value.old, `${prefix}_${value.local}`);
  }
  let canonical = text;
  for (const [oldValue, newValue] of [...replacements.entries()].sort(([left], [right]) => right.length - left.length)) {
    const escaped = oldValue.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&");
    canonical = canonical.replace(new RegExp(`(?<![A-Za-z0-9_-])${escaped}(?![A-Za-z0-9_-])`, "gu"), newValue);
  }
 ...
 */
export async function materializePayload(targetDir, { stripScripts = false } = {}) {
  const manifest = JSON.parse(await readFile(join(distPackageDir, "package.json"), "utf8"));
  await buildDistLib();
  await mkdir(targetDir, { recursive: true });
  for (const entry of ["lib", "cordis.patch.yml", "README.md"]) {
    await cp(join(distPackageDir, entry), join(targetDir, entry), { recursive: true });
  }
  ...
  await writeFile(join(targetDir, "package.json"), `${JSON.stringify(deriveShippedManifest(manifest, { stripScripts }), null, 2)}\n`, "utf8");
  return JSON.parse(await readFile(join(targetDir, "package.json"), "utf8"));
}

/** Recursive byte-for-byte comparison used by the drift check and tests. */
export async function directoriesDiffer(left, right, normalize, { skip = [] } = {}) {
  ...
  for (const name of leftNames) {
    ...
    if (sa.isDirectory() !== sb.isDirectory()) return `type mismatch at ${name}`;
    if (sa.isDirectory()) {
      const nested = await directoriesDiffer(a, b, normalize);
      if (nested !== null) return nested;
      continue;
    }
    if (sa.size !== sb.size && normalize === undefined) return `size mismatch at ${name}`;
