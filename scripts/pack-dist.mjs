...
let manifest;
try {
	manifest = JSON.parse(await readFile(join(distDir, "package.json"), "utf8"));
} catch (cause) {
	throw new Error(`dist package manifest is not valid JSON: ${String(cause)}`, {
		cause,
	});
}
...
execFileSync("npm", ["pack", "--silent", "--pack-destination", releaseDir], {
	cwd: staged,
});

// Verify tarball contents match the staged payload exactly.
const tgzListing = execFileSync("tar", ["-tzf", join(releaseDir, tgzName)], {
	encoding: "utf8",
})
	.split("\n")
	.map((line) => line.trim())
...
if (missing.length > 0)
	throw new Error(`tgz missing staged files: ${missing.join(", ")}`);
...
await writeFile(
	join(releaseDir, "pack-manifest.json"),
	`${JSON.stringify(summary, null, 2)}\n`,
	"utf8",
);
console.log(
	`packed ${list.length} files -> ${join(relative(repoRoot, releaseDir), tgzName)}`,
);
