...
function sourcePrompt(prepare: Awaited<ReturnType<typeof loadPrepare>>, sources: readonly { nodeId?: string; path: string; startLine?: number; endLine?: number; content: string }[], target: ExplainReferenceTarget): string { const targetText = JSON.stringify(target); const targetInstruction = target.kind === "none" ? "No optional reference selected; use only the prepared source ranges; do not call ici_explain_list or ici_explain_read." : target.kind === "file" ? `Selected file ${target.path}; use ici_explain_read only if needed, then submit (listing is optional).` : `Selected directory ${target.path || "."}; use ici_explain_list/read only if needed, then submit.`; return [`You are a dedicated read-only explanation agent for API ${prepare.api.name}. Workspace-relative reference target: ${targetText}. ${targetInstruction} Use only the three tools ici_explain_list, ici_explain_read, ici_explain_submit. Never use absolute paths, shell/network/write tools, or any other tool. Read only useful text files (at most 20 files and 256 KiB total), then call ici_explain_submit with a valid object; schema-invalid may be corrected and retried up to three attempts. Submit exactly technical/business strings, flow string[], and evidence string[]. Evidence must use only prepared/read relative paths as path#N or path#N-M; # is mandatory, never use : or objects. Never emit a final answer instead of submit.`, `Prepare metadata: ${JSON.stringify({ api: prepare.api, manifest: prepare.manifest, prepareId: prepare.prepareId, contextHash: prepare.contextHash })}`, `Prepare call chain: ${JSON.stringify(prepare.callChain)}`, ...sources.map(source => `--- prepared source ${source.nodeId ?? "api"} ${source.path}#${source.startLine ?? 1}-${source.endLine ?? 1}\n${source.content}`)].join("\n"); }
...
async function runDedicatedAgent(ctx: Context, root: string, job: Awaited<ReturnType<typeof readJobRecord>> & {}, prepare: Awaited<ReturnType<typeof loadPrepare>>, sources: readonly { nodeId?: string; path: string; startLine?: number; endLine?: number; content: string }[], signal: AbortSignal, parent?: AgentLike): Promise<{ analysis: { technical: string; business: string; flow: string[]; evidence: string[] }; folderReads: readonly { path: string; sha256: string }[]; childSessionId: string }> {
  const agents = ctx.get("agents") as AgentsFace | undefined; if (!agents?.create) throw new Error("model-failed");
  const target = referenceTargetOf(job); await assertReferenceTarget(root, target); const initialPrompt = sourcePrompt(prepare, sources, target); const basePromptBytes = Buffer.byteLength(initialPrompt, "utf8"); if (basePromptBytes > MAX_EXPLAIN_PROMPT_BYTES) throw new Error("input-too-large");
  ...
  const setup = (childCtx: any): void => {
    ...
    tools.register({ name: "ici_explain_read", description: target.kind === "none" ? "No optional reference selected; do not call this tool. It returns folder-forbidden without filesystem access." : "Read one file under the selected workspace-relative reference target, subject to strict byte and call limits; use only when needed.", parameters: toolSchema({ path: { type: "string", description: "Workspace-relative file path." } }, ["path"]), output: { schema: toolSchema({ path: { type: "string" }, content: { type: "string" }, bytes: { type: "integer" }, sha256: { type: "string" } }, ["path", "content", "bytes", "sha256"]), render: (_a: unknown, value: unknown) => toolOutput(value) }, execute: async (raw: Record<string, unknown>) => { if (++readCalls > MAX_READ_FILES) { toolFailure = true; toolFailureReason = "tool-limit"; childRef?.cancel("tool-failed"); throw new Error("tool-limit"); } try { if (typeof raw.path !== "string") throw new Error("folder-forbidden"); const file = await readReferenceText(root, target, raw.path); readBytes += file.bytes; if (readBytes > Math.min(MAX_REFERENCE_BYTES, Math.max(0, MAX_EXPLAIN_PROMPT_BYTES - basePromptBytes))) { toolFailureReason = "input-too-large"; throw new Error("input-too-large"); } verifiedPaths.add(file.path); folderReads.push({ path: file.path, sha256: file.sha256 }); reserveToolOutput(file); return file; } catch (error) { const message = error instanceof Error && ["tool-limit", "folder-oversize", "input-too-large", "folder-forbidden"].includes(error.message) ? error.message : "folder-forbidden"; if (!(target.kind === "none" && message === "folder-forbidden")) { toolFailure = true; toolFailureReason = message; childRef?.cancel("tool-failed"); } throw new Error(message); } } });
    tools.register({ name: "ici_explain_submit", description: "Submit exactly one strict aggregate API explanation; this is the only completion boundary. technical/business are strings, flow is string[], and evidence is string[] using relative path#N or path#N-M (use #, never : or objects).", parameters: toolSchema({ technical: { type: "string", minLength: 1, maxLength: 12000, description: "Factual technical explanation." }, business: { type: "string", minLength: 1, maxLength: 12000, description: "Factual business explanation." }, flow: { type: "array", minItems: 0, maxItems: MAX_FLOW_ITEMS, items: { type: "string", minLength: 1, maxLength: 500 }, description: "Ordered factual flow steps as strings." }, evidence: { type: "array", minItems: 1, maxItems: 64, items: { type: "string", minLength: 1, maxLength: 400, pattern: EVIDENCE_PATTERN }, description: "Relative prepared/read source references; each must be path#N or path#N-M, never path:N or an object." } }, ["technical", "business", "flow", "evidence"]), output: { schema: toolSchema({ accepted: { type: "boolean" } }, ["accepted"]), render: (_a: unknown, value: unknown) => toolOutput(value) }, execute: async (raw: Record<string, unknown>) => { if (toolFailure) throw new Error(toolFailureReason ?? "model-failed"); if (submittedOnce) { toolFailure = true; toolFailureReason = "schema-invalid"; childRef?.cancel("tool-failed"); throw new Error("schema-invalid"); } if (!strictAnalysis(raw, target, verifiedPaths)) { toolFailureReason = "schema-invalid"; if (unsafeAnalysis(raw) || ++submitAttempts >= MAX_SUBMIT_ATTEMPTS) { toolFailure = true; childRef?.cancel("tool-failed"); throw new Error("schema-invalid"); } throw new Error(SUBMIT_REPAIR); } submittedOnce = true; resolveSubmitted(raw as { technical: string; business: string; flow: string[]; evidence: string[] }); return { accepted: true }; } });
...
export async function processConfirmedJob(llm: LlmExplainFace, root: string, jobId: string, signal: AbortSignal, ctx?: Context, parent?: AgentLike): Promise<void> {
  ...
  try {
    ...
    const prompt = sourcePrompt(prepare, sources, target); if (Buffer.byteLength(prompt, "utf8") > MAX_EXPLAIN_PROMPT_BYTES) throw new Error("input-too-large"); signal.throwIfAborted(); const latestBeforeChild = await readJobRecord(root, jobId); if (!latestBeforeChild || latestBeforeChild.status !== "running") return;
    const completed = await runDedicatedAgent(ctx, root, running, prepare, sources, signal, parent); const published = await publishFinal(root, running, completed.analysis, signal, completed.folderReads, target, completed.childSessionId); if (!published) return;
  } catch (cause) {
