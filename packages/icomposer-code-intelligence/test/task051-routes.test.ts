...
async function fixture() {
  ...
  const raw = await buildGraph(root, [{ name: "RouteAPI", type: "api", sourcePath: join(root, "src", "RouteAPI.groovy") }]); const graph: any = { nodes: new Map(raw.nodes.map(node => [node.id, node])), edges: raw.edges, manifest: { sourceFingerprint: "f".repeat(64) } }; await mkdir(join(root, ".metadata/icomposer/ici/graph/current"), { recursive: true }); await writeFile(join(root, ".metadata/icomposer/ici/graph/current/manifest.json"), JSON.stringify({ engineVersion: ICI_ENGINE_VERSION, sourceFingerprint: graph.manifest.sourceFingerprint, graphDigest: computeGraphDigest(graph) }));
...
test("TASK-054 confirm accepts a workspace-relative file reference target", async () => {
  const fx = await fixture(); const routes: any[] = []; const ctx: any = new Context(); ctx.provide("webServer", { register(route: any) { routes.push(route); return () => undefined; } }); ctx.provide("workspaceBinding", { list: async () => ({ ok: true, value: [{ workspaceId: "route", canonicalPath: fx.root }] }) }); ctx.provide("llm", { listProviders: () => [{ id: "mvp" }], resolveModelInfo: async () => undefined }); ctx.provide("iciEngine", {}); ctx.provide("iciExplainScheduler", { poke: () => undefined, cancelJob: async () => false }); const fiber: any = await ctx.plugin(ExplainRoutesService); await fiber.await(); const handler = routes[0].handler;
  try {
    const confirmRes = response(); await handler(req("POST", `/api/icomposer-workbench/ici/explain/jobs/${fx.job.jobId}/confirm`, { provider: "mvp", model: "mvp-model", referenceTarget: { path: "ref_doc/nested/guide.md", kind: "file" }, docs: [], consent: true }), confirmRes); const confirmed = decode(confirmRes); assert.equal(confirmed.ok, true); assert.deepEqual(confirmed.result.referenceTarget, { path: "ref_doc/nested/guide.md", kind: "file" }); assert.deepEqual((await readJobRecord(fx.root, fx.job.jobId))?.referenceTarget, { path: "ref_doc/nested/guide.md", kind: "file" });
    const folderRes = response(); await handler(req("GET", `/api/icomposer-workbench/ici/explain/jobs/${fx.job.jobId}/folder`), folderRes); assert.equal(decode(folderRes).result.folderPath, "ref_doc/nested");
    const bad = response(); await handler(req("POST", `/api/icomposer-workbench/ici/explain/jobs/${fx.job.jobId}/confirm`, { provider: "mvp", model: "mvp-model", referenceTarget: { path: "ref_doc/nested/guide.xml", kind: "file" }, docs: [], consent: true }), bad); assert.equal(decode(bad).error.code, "confirmation-invalid");
  } finally { await fiber.dispose(); await fx.cleanup(); }
});
