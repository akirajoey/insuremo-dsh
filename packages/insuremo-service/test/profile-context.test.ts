...
import {
  ...
  PROFILE_DIGEST_SECTION, eventDigest, compactCheckpointSource, shellQuote,
...
test("TASK-045 profile policy migration upgrades a legacy same-digest event once", async () => {
  const fx = await mountFixture(makeProfile("portal:microsite"));
  try {
    const agent: HistoryAgent = { session: { events: [{
      type: "user/message",
      data: { content: [{ type: "text", text: "InsureMO active profile: portal:microsite." }], source: {
        kind: "plugin", plugin: "icomposer-current-profile",
        sections: [{ name: PROFILE_DIGEST_SECTION, text: `{d=${shortDigest("portal:microsite")}}` }],
      } },
    }] } };
...
test("TASK-045 shellQuote is execution-safe for profile command arguments", async () => {
  ...
  assert.equal(await run("evil'x"), "evil'x");
  assert.equal(await run("space value; printf BAD"), "space value; printf BAD");
  assert.equal(await run("$(printf BAD)"), "$(printf BAD)");
});
