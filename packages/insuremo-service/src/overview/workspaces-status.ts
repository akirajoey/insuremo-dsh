...
/**
 * Bounded local read of the ICI explain-state marker
 * (`<DSH_HOME>/ici/<hash>/explain-state.json`), mirroring the ici package's
 * schema check. Kept local because this package's tsconfig rootDir cannot
 * include sibling sources; the file format is a frozen v1 contract.
 */
async function localReadExplainState(canonicalPath: string, workspaceId: string): Promise<boolean> {
  try {
    ...
    const text = await readFile(join(dshHome, "ici", hash, "explain-state.json"), "utf8");
