...
export const WORKSPACES_STATUS_URL =
	"/api/icomposer-workbench/insuremo/overview/workspaces/status" as const;

export function parseWorkspaceHealthRows(
	value: unknown,
): readonly WorkspaceHealthRow[] | null {
	...
	for (const item of list.slice(0, 100)) {
		...
		const state =
			row.autoBindState === "bound" || row.autoBindState === "pending"
				? row.autoBindState
				: "none";
		rows.push({
...
function Glyphs(props: {
	...
	const graphLabel = props.row.graphReady
		? props.t("health.graphReady")
		: props.t("health.graphNotReady");
	const explainLabel = props.row.explainReady
		? props.t("health.explainReady")
		: props.t("health.explainNotReady");
	return (
		<span
			className={css.rowIcons}
 ...
 */
export class WorkspaceHealth extends Component<
	WorkspaceHealthProps,
	{ rows: readonly WorkspaceHealthRow[] }
> {
	...
	override componentDidMount(): void {
		...
		if (this.#driverRef?.ownerDocument !== undefined) {
			this.#observer.observe(this.#driverRef.ownerDocument.body, {
				childList: true,
				subtree: true,
			});
		}
	...
	private syncRows(): void {
		...
		const treeitems = Array.from(
			doc.querySelectorAll<HTMLElement>('[role="treeitem"][aria-expanded]'),
		);
		for (const treeitem of treeitems) {
			...
			const candidates = rowsInOrder.filter(
				(candidate) => candidate.displayName === label,
			);
			if (candidates.length === 0) continue;
			const occurrence = this.#occurrenceCounter.get(label) ?? 0;
			this.#occurrenceCounter.set(label, occurrence + 1);
			const existingHost = treeitem.querySelector<HTMLElement>(
				`[${HOST_ATTR}]`,
			);
			const existingId =
				this.#rowIds.get(treeitem) ??
				existingHost?.getAttribute("data-icomposer-workspace-id");
			const preserved =
				existingId === undefined || existingId === null
					? undefined
					: candidates.find(
							(candidate) => candidate.workspaceId === existingId,
						);
			const available = candidates.filter(
				(candidate) => !seenIds.has(candidate.workspaceId),
			);
			const row =
				preserved !== undefined && !seenIds.has(preserved.workspaceId)
					? preserved
			...
			if (!row.detected) {
				const staleId = existingHost?.getAttribute(
					"data-icomposer-workspace-id",
				);
				const stalePort =
					staleId === undefined || staleId === null
						? undefined
						: this.#ports.get(staleId);
				if (
					staleId !== undefined &&
					staleId !== null &&
					stalePort?.host === existingHost
				)
					this.dropPort(staleId);
			...
			if (port !== undefined && port.host.parentElement !== treeitem)
				treeitem.appendChild(port.host);
			if (
				port === undefined &&
				treeitem.querySelector(`[${HOST_ATTR}]`) === null
			) {
				...
				const anchor =
					treeitem.querySelector('[class*="rowActions"]') ??
					treeitem.lastElementChild;
