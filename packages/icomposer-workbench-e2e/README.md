...
```bash
pnpm --filter @icomposer/icomposer-workbench-e2e run e2e            # 10 步全链
...
1. **profile-composition** — setup-profile 安装 + dump 断言（13 插件行、关键 inject 行、退役包不安装）
...
9. **ici-cleanup** — cleanupPlan/Apply + 诊断干净
10. （--stability）**stability-large-build** — 5001 资产 fixture → 10002 节点 + queryApi maxNodes=1000 truncated
11. （--stability）**stability-kill-mid-build** — 子进程 SIGKILL 中断重建 → current 快照字节完整 + 无 staging 残留

报告：`<DSH_HOME>/e2e-report.json`（每步 {step, ok, durationMs, counts}；任何失败非零退出）。

## 审计脚本

- `scripts/audit-compat.mjs` — Harness HEAD 固定/工作树干净/全部 @deepseek-ai peer 解析到固定 checkout（tsconfig.base 路径或 harness-shims 或包级 tsconfig 路径）/bundle 13 插件依赖齐/lockfile 快照 → `docs/compat-audit.json`
