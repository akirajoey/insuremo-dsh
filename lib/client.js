...
/** The globe mark rendered into `sidebar.brand.mark` /
* `conversation.hero.brand.mark` (slot prop contract: `{ size?: number }`). */
...
function apply$2(ctx) {
	...
	ctx.slots.inject("sidebar.brand.name", () => ctx.slots.inject("conversation.hero.brand.mark", function* () {
		yield ctx.slots.register({ name: "sidebar.brand.mark" }, InsuremoBrandMark);
		yield ctx.slots.register({ name: "sidebar.brand.name" }, InsuremoBrandName);
		yield ctx.slots.register({ name: "conversation.hero.brand.mark" }, InsuremoBrandMark);
	}));
